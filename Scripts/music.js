...
xhr.open('GET', './Resources/musime.jpg', true);
xhr.responseType = 'arraybuffer';
xhr.onload = function (e) {
    var uInt8Array = new Uint8Array(this.response);
    condb = new SQL.Database(uInt8Array);
    contents = condb.exec("SELECT word FROM rubymusic where ruby='a1' ");
...
function addSelRuby(ruby) {
    ...
    contents = condb.exec("SELECT word FROM " + opttable + " WHERE " + optruby + " = '" + ruby.replace(/\'/g, "''").toLowerCase() + "' ");
    ...
    if (sugCB) {
        contents = condb.exec("select word, ruby from " + opttable + " where " + optruby + " like '" + ruby.replace(/\'/g, "''").toLowerCase() + "%' and " + optruby + "!='" + ruby + "' ");
        if (contents.length != 0) {
            if (cubo.length > 0)
                cubo.push(ruby);
            for (i = 0; i < contents[0].values.length; i++) {
...
function selPhone(phrase, maxlevel, defa){
    ...
    for (k = 0; k != word.length; k++) {
        ...
        sql = "select " + optruby + " from " + opttable + " where word='" + fullchar + "'";
...
function selChar(phrase, maxlevel, defa) {
    ...
    for (k = 0; k != word.length; k++) {
        ...
        sql = "select word from " + opttable + " where " + optruby + "='" + fullchar + "'";
        contents = condb.exec(sql);
        if (contents.length != 0) {
            sss = contents[0].values[0][0];
            if (ext) {
    ...
    if ($('#keyboard').css("display") == "block") {
        switch (quocngu) {
            ...
            case 24:
                ...
                $('#K86').html("<span style='color: #d22e9f;'>𕈧  </span><span style='color: #4fd454;'>𕈰</span><br><span style='color: #ff66b4;'>𕉆  </span>𕈦");
