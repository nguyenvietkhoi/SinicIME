...
for (var ise=0; ise<sentence.length; ise++) {	
    ...
    for (let len = Math.min(5, words.length - i); len > 0; len--) {
	  ...
	  contents = condb3.exec("SELECT viet, " + optta + " FROM trans" + optta + " WHERE " + optta + " = '" + compound.toLowerCase().replace(/\'/g, "''") + "'");
      if (contents.length != 0) {
        outputarr.push(contents[0].values[0][0]);
        i += len;
        matched = true;
        break;
      }
